...
gadgets = {
    ...
    { config_id = 14, gadget_id = 70211002, pos = { x = 2723.4087, y = 195.24835, z = -1681.6371 }, rot = { x = 0.0, y = 271.5409, z = 0.0 }, level = 1, drop_tag = "战斗低级蒙德", state = GadgetState.ChestStart, isOneoff = false, persistent = false, explore = { name = "chest", exp = 1 }, area_id = 1 },
    { config_id = 15, gadget_id = 70211012, pos = { x = 2723.3916, y = 196.20367, z = -1681.6367 }, rot = { x = 0.0, y = 271.5409, z = 0.0 }, level = 1, drop_tag = "战斗低级蒙德", state = GadgetState.ChestStart, isOneoff = false, persistent = false, explore = { name = "chest", exp = 1 }, area_id = 1 },
    { config_id = 16, gadget_id = 70211022, pos = { x = 2723.4424, y = 197.24252, z = -1681.638 }, rot = { x = 0.0, y = 271.5409, z = 0.0 }, level = 1, drop_tag = "战斗中级蒙德", state = GadgetState.ChestStart, isOneoff = false, persistent = false, explore = { name = "chest", exp = 1 }, area_id = 1 },
    { config_id = 17, gadget_id = 70211032, pos = { x = 2723.4414, y = 200.60762, z = -1681.638 }, rot = { x = 0.0, y = 271.5409, z = 0.0 }, level = 1, drop_tag = "战斗高级蒙德", state = GadgetState.ChestStart, isOneoff = false, persistent = false, explore = { name = "chest", exp = 1 }, area_id = 1 }
}
...
-- 变量
variables = {
	-- 用一个变量表示解锁进度最后用触发条件函数判断是不行的 应该要用几个独立的变量在每个触发操作函数判断
	{ config_id = 30, name = "Obelisk_Windy", value = 0, no_refresh = false },
	{ config_id = 31, name = "Obelisk_Fire", value = 0, no_refresh = false },
	{ config_id = 32, name = "Obelisk_Ray", value = 0, no_refresh = false },
}
...
suites = {
    {
        ...
        gadgets = { },
        regions = { },
        triggers = { "ANY_MONSTER_DIE_3", "GADGET_STATE_CHANGE_7", "GADGET_STATE_CHANGE_8", "GADGET_STATE_CHANGE_9", "VARIABLE_CHANGE_13" },
        rand_weight = 100
    }
}
...
function action_EVENT_GADGET_STATE_CHANGE_7(context, evt)
	-- 变量 "Obelisk_Windy" + 1
	if 0 ~= ScriptLib.ChangeGroupVariableValue(context, "Obelisk_Windy", 1) then
		ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : change_group_variable_key_failed_7")
		return -1
	end

	-- 判断 三个方碑是否都已解锁
	if 3 <= ScriptLib.GetGroupVariableValue(context, "Obelisk_Windy") + ScriptLib.GetGroupVariableValue(context, "Obelisk_Fire") + ScriptLib.GetGroupVariableValue(context, "Obelisk_Ray") then
		-- 创建 物件 - 宝箱
...
function action_EVENT_GADGET_STATE_CHANGE_8(context, evt)
	-- 变量 "Obelisk_Fire" + 1
	if 0 ~= ScriptLib.ChangeGroupVariableValue(context, "Obelisk_Fire", 1) then
		ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : change_group_variable_key_failed_7")
		return -1
	end
...
function action_EVENT_GADGET_STATE_CHANGE_9(context, evt)
	-- 变量 "Obelisk_Ray" + 1
	if 0 ~= ScriptLib.ChangeGroupVariableValue(context, "Obelisk_Ray", 1) then
		ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : change_group_variable_key_failed_7")
		return -1
	end
